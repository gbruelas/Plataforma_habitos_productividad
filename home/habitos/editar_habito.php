<?php
    ...
    try {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Determinar que opción de personalizada se selecciono
            if ($id_frecuencia === 4) {
                if ($opcion_personalizada == 'dias') {
                    // Si selecciono dias, obtenemos los checkboxes marcados
                    $dias_seleccionados = $_POST['dias'] ?? [];
                    // Asegurarnos que al menos seleccionó un día si eligió esta opción
                    if (empty($dias_seleccionados)) {
                        $_SESSION['error'] = "Debes seleccionar al menos un día de la semana";
                        header("Location: editar_habito.php?id=$id"); // Para que lo redireccione al mismo habito
                        exit;
                    }
                } elseif ($opcion_personalizada == 'cada_x_dias') {
                    // Si selecciono cada X días, obtenemos el valor numerico, y luego validamos que el valor ingresado sea valido
            ...
            // Actualizar hábito, siempre limpiamos cada_cuantos_dias si no es personalizada o es personalizada por dias
            ...
            // Siempre eliminamos los dias existentes (si los hay)
            $pdo->prepare("DELETE FROM dias_habito WHERE id_habito = ?")->execute([$id]);
            
            // Si es frecuencia personalizada por días, insertamos los nuevos
        ...
        elseif (isset($_GET['id']) && is_numeric($_GET['id'])) {
            ...
            // Obtenemos los datos básicos del hábito
            ...
            if ($habito && $habito['id_frecuencia'] == 4) {
